...
void functions::loopUdpServer(unsigned short port, std::map <sf::IpAddress, ServerPlayer>& ServerBase){
    ...
    packet.clear();
    packet << ServerBase[sender].client_name << ServerBase[sender].ship.getCoordinates().first << ServerBase[sender].ship.getCoordinates().second << ServerBase[sender].ship.ANGLE();
    for(int i = 0; i < ServerBase.size(); ++i) {
